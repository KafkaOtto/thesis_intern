...
helm install -f "$BASE_DIR/deployment/reranker/k8s/bm25s/values.yaml" reranker-bm25s "$BASE_DIR/deployment/reranker/k8s/bm25s"
helm install -f "$BASE_DIR/deployment/backend/k8s/values-t2-reranking.yaml" chat-backend "$BASE_DIR/deployment/backend/k8s"
