...
def perform_paired_ttests_with_effect_size(df, group, column, label, one_tailed=False):
    ...
    for item_group in group_orders_combo:
        for group_t, group_vs in item_group.items():
            for group_v in group_vs:
                if any(group_t == g_t and group_v in g_vs for group_dict in group for g_t, g_vs in group_dict.items()):
                    treatment_labels.append(TREATMENT_MAPPING.get(group_t, group_t))
                    variable_labels.append(LABEL_MAPPING.get(group_v, group_v))
...
treatment_labels, variable_labels, energy_percentage_changes = perform_paired_ttests_with_effect_size(df, group=draw_groups, column="energy_total", label="energy(J)", one_tailed=True)
treatment_labels, variable_labels, accuracy_percentage_changes = perform_paired_ttests_with_effect_size(df, group=draw_groups, column="accuracy", label="accuracy(%)", one_tailed=False)
treatment_labels, variable_labels, latency_percentage_changes = perform_paired_ttests_with_effect_size(df, group=draw_groups, column="latency", label="latency(ms)", one_tailed=False)
...
plot_metrics_comparison(
    ...
    save_path=f"plots/all_metrics_comparison_combos.pdf"
)