...
helm install pgvector "$BASE_DIR/deployment/postgre/helm-pgvector/helm/pgvector" --set postgresql.password=root
...
SCHEMA_SCRIPT="$BASE_DIR/deployment/postgre/scripts/2__schema_script_embd768.sql"
...
kubectl apply -f "$BASE_DIR/deployment/embedding/k8s/e5_base_v2/pvc.yaml"
kubectl apply -f "$BASE_DIR/deployment/embedding/k8s/e5_base_v2/deployment_gpu.yaml"
kubectl apply -f "$BASE_DIR/deployment/embedding/k8s/e5_base_v2/service.yaml"
...
helm install -f "$BASE_DIR/deployment/data_importer/k8s/values-t3-e5-base-v2.yaml" importer "$BASE_DIR/deployment/data_importer/k8s"
...
kubectl delete -f "$BASE_DIR/deployment/embedding/k8s/e5_base_v2/deployment_gpu.yaml"
kubectl delete -f "$BASE_DIR/deployment/embedding/k8s/e5_base_v2/service.yaml"
kubectl delete -f "$BASE_DIR/deployment/embedding/k8s/e5_base_v2/pvc.yaml"
