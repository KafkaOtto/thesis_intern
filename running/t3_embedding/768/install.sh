...
kubectl apply -f "$BASE_DIR/deployment/embedding/k8s/e5_base_v2/deployment.yaml"
kubectl apply -f "$BASE_DIR/deployment/embedding/k8s/e5_base_v2/service.yaml"

helm install -f "$BASE_DIR/deployment/backend/k8s/values-t3-e5-base-v2.yaml" chat-backend "$BASE_DIR/deployment/backend/k8s"
